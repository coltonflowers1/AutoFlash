...
def create_flashcard_set():
    ...
    if not set_name:
        return "Set name is required", 400

    try:
        # Assuming set names are unique. Adjust as per your requirements.
        existing_set = flashcard_sets_collection.find_one({"name": set_name})
        if existing_set:
            return "A set with this name already exists", 409

        # Create the new flashcard set
        new_set = {"name": set_name, "flashcards": []}
        flashcard_sets_collection.insert_one(new_set)

        return "Flashcard set created successfully"
    except Exception as e:
        return str(e), 500


@route("/get-flashcard-sets")
def get_flashcard_sets():
    # ... code to fetch flashcard set names ...
    flashcard_sets = flashcard_sets_collection.find()
    sets_list = [
        {
            "id": str(set["_id"]),  # Convert ObjectId to string
            "name": set["name"],  # Assuming each set has a 'name' field
        }
...
def fetch_set_info(set_id):
    ...
    set_info = flashcard_sets_collection.find_one({"_id": set_id})
...
def fetch_cards_for_set(set_id):
    ...
    try:
        ...
        set_data = flashcard_sets_collection.find_one(
            {"_id": ObjectId(set_id)}, {"_id": 0, "flashcards": 1}
        )

        if set_data and "flashcards" in set_data:
            # Return the list of flashcards
            return [{"frontText":set["front text"],"backText":set["back text"],"cardId":set["card_id"]} for set in set_data["flashcards"]]
        else:
...
def delete_flashcard(set_id):
    ...
    try:
        ...
        result = flashcard_sets_collection.update_one(
            {"_id": ObjectId(set_id)}, {"$pull": {"flashcards": {"card_id": flashcard_id}}}
        )
